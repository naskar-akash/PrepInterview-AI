    ...
    try {
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          // updating user
          dispatch(setUserData(verification.user.credits))
          // toast message
          toast.success(
            `${verification.message} ${plan.credits} credits added 🎉`,
            {
              style: {
                background: "#16a34a",
                color: "#fff",
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-pink-50 py-16 px-6">
      <div className="max-w-6xl mx-auto mb-14 flex items-start gap-4">
        <button
          onClick={() => navigate("/")}
          className="p-3 mt-2 rounded-full bg-white shadow hover:shadow-md transition"
        >
        ...
        {plansArray.map((plan) => {
          ...
          return (
            ...
            >
              ...
              {/* Plan name */}
              <h3 className="text-xl font-semibold text-gray-800 text-shadow-md">
                {plan.name}
              </h3>
              {/* Price */}
              <div className="mt-4">
